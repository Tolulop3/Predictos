...
def validate_pick_live(pick: dict, model_prob: float) -> dict:
    ...
    # Block extreme longshots AND near-certainties at the validation gate
    # yes_price > MAX_MARKET_PROB (92%): near-certainty — bad Kelly sizing
    # entry_price < 5%: the side being bet is too extreme
    if scored_price > config.MAX_MARKET_PROB or scored_price < config.MIN_MARKET_PROB:
        return {
            **pick,
            ...
            "live_passes":       False,
            "validation_status": "extreme_probability",
            "validation_note":   f"Market price {scored_price:.1%} outside valid range ({config.MIN_MARKET_PROB:.0%}-{config.MAX_MARKET_PROB:.0%})",
...
# Convenience alias used by tests and integration checks
validate_pick = validate_pick_live
