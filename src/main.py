...
from src.tick_store    import save_ticks_batch, enrich_markets_from_ticks, tick_store_stats, _backdate_single_ticks
...
def run_pipeline(save_scores: bool = True) -> dict:
    ...
    try:
        ...
        n_total_picks     = n_resolved + learning_result.get("n_unresolved", 0)
    ...
    try:
        _backdate_single_ticks()   # one-shot bootstrap: makes 24h momentum available immediately
    ...
    for market in markets:
        try:
            ...
            # When no trained model: use calibrated_prob directly
            # When trained: blend with news probability
            if model.is_trained:
            ...
            # Compute meaningful confidence score based on signal agreement
            ml_confidence = model.compute_confidence(market, model_prob, sent_score)
