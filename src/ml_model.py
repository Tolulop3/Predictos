...
class PredictOSModel:
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.is_trained and self.model and XGB_AVAILABLE:
            ...
            # Strategy: use sentiment to push model_prob away from market price.
            # A pure blend of yes_price + sentiment hugs market price → edge always tiny.
            # Instead: treat sentiment as a directional signal that adjusts market price.
            if len(X) == 0:
                return np.array([])
            yes_price  = X[:, 0]       # market's current probability
            sent_score = X[:, 2]       # 0–1 sentiment score (0.5 = neutral)
            momentum   = X[:, 5]       # 24h momentum (price change)

            # Sentiment adjustment: how far above/below neutral is sentiment?
            # sent_score 0.7 → +0.2 adjustment, sent_score 0.3 → -0.2 adjustment
            sent_adj = (sent_score - 0.5) * 0.40

            # Momentum adjustment: recent price direction
            mom_adj = momentum * 1.5

            # Base: market price + adjustments, clipped to [0.05, 0.95]
            model_prob = yes_price + sent_adj + mom_adj
