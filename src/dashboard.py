...
<div class="pick-card" style="border-left:3px solid {side_color};background:{side_bg}">
  ...
  <div class="stats-grid">
    ...
    <div class="stat"><label>Kelly</label><value>{kelly_f:.1%}</value></div>
    <div class="stat"><label>Suggest</label><value>${pos_usd:.0f}</value></div>
    <div class="stat"><label>Days</label><value>{days:.0f}d</value></div>
    <div class="stat"><label>ML</label><value>{ml_conf:.0%}</value></div>
  </div>
  {ret_html}
  {drift_note}
  {flags_html}
...
@media(max-width:900px){{
  .hdr{{grid-template-columns:1fr;padding:16px}}.hdr-right{{text-align:left}}
  .kpi-bar{{grid-template-columns:repeat(3,1fr)}}.kpi:nth-child(3){{border-right:none}}
