...
def data_api_price_history(market_id: str,
    ...
    data-api /prices returns 404 for all markets — removed entirely.
    CLOB is the reliable source for historical price data.
    """
    import time as _time
    end_ts   = int(_time.time())
    start_ts = end_ts - 4 * 86400  # 4 days back

    data = _get(
        f"{config.POLYMARKET_CLOB_BASE}/prices-history",
        params={"market": market_id, "startTs": start_ts,
                "endTs": end_ts, "fidelity": fidelity},
    )
    if not data:
        return []
    history = data.get("history", [])
    return sorted(history, key=lambda h: h.get("t", h.get("ts", 0)))
...
def data_api_resolution(condition_id: str) -> Optional[dict]:
    """
    data-api /resolution returns 404 — removed.
    Resolution is checked via CLOB market state in check_polymarket_resolution().
    Kept as stub so tracker import doesn't break.
    """
    return None
...
def fetch_kalshi_markets(limit: int = 200) -> list[dict]:
    """
    Fetch Kalshi markets. Kalshi now requires auth for most endpoints.
    Returns empty list gracefully if unauthorized — Polymarket covers the gap.
    """
    markets = []
    cursor  = None
    for _ in range(5):
        ...
        if not data:
            # 401 = auth required, log once and stop
            log.info("Kalshi: auth required or unavailable — skipping (Polymarket covers this)")
            break
...
def fetch_manifold_markets(limit: int = 100) -> list[dict]:
    """
    Fetch Manifold markets for signal/sentiment coverage.
    Note: Manifold is play-money only (real money sunset March 2025).
    Useful for additional market sentiment signal, not real trading.
    """
    markets = []
    before  = None
    for _ in range(3):
        # Manifold v0: only use limit + before for pagination
        # filter and sort params vary by version — keep minimal to avoid 400
        params = {"limit": limit}
        if before:
            params["before"] = before
        data = _get(f"{config.MANIFOLD_BASE}/markets", params=params)
        if not data or not isinstance(data, list):
            log.info("Manifold: no data returned")
            break
        for m in data:
            # Filter to binary markets client-side
        ...
        time.sleep(0.3)
    log.info(f"Manifold: {len(markets)} markets (play-money signal only)")
    return markets
...
def _parse_manifold(m: dict) -> Optional[dict]:
    ...
    return {
        ...
        "source":          "manifold",
        "sentiment_only":  True,   # never a pick — play-money only, used for sentiment signal
        "question":        title,
        "category":        _classify_topic(title),
...
def fetch_news_articles(query: str = "", days: int = 3) -> list[dict]:
    """
    Fetch news articles across targeted topic categories.
    Uses multiple focused queries instead of one blank query so that
    most prediction markets get relevant article matches.
    """
    # Topic queries covering the main Polymarket categories
    TOPIC_QUERIES = [
        "Federal Reserve interest rates inflation",
        "Bitcoin crypto Ethereum market",
        "Ukraine Russia ceasefire war",
        "Trump tariffs trade China economy",
        "election president Congress senate",
        "AI artificial intelligence technology",
        "oil energy stock market",
    ]

    all_articles: list[dict] = []
    seen_urls: set[str] = set()

    def _add_articles(data: dict) -> None:
    ...
    if config.NEWS_API_KEY:
        ...
        # Fetch top headlines first (broad coverage, costs 1 request)
        base_params = {
        ...
        if query:
            base_params["q"] = query
        data = _get("https://newsapi.org/v2/everything", params=base_params)
        _add_articles(data)

        # Fetch topic-specific queries (only if no explicit query override)
        if not query:
            for topic_q in TOPIC_QUERIES:
                ...
                data = _get("https://newsapi.org/v2/everything", params=topic_params)
                _add_articles(data)

    # RSS feeds — always fetch, good for sports and regional coverage
    for feed_url in config.RSS_FEEDS + config.SPORTS_RSS_FEEDS:
        try:
    ...
    log.info(f"News: {len(all_articles)} articles (query='{query or 'multi-topic'}')")
    return all_articles
...
def _classify_topic(text: str) -> str:
    ...
    rules = {
        ...
        "sports":      [
            # American sports
            "nfl","nba","mlb","nhl","super bowl","world series","nba finals",
            "stanley cup","ncaa","march madness","college football",
            # Global football / soccer
            "premier league","la liga","serie a","bundesliga","ligue 1",
            "champions league","europa league","world cup","euro ","copa",
            "fa cup","league cup","efl","mls","eredivisie",
            # Other sports
            "ufc","mma","formula 1"," f1 ","grand prix","wimbledon","us open",
            "australian open","french open","masters","pga","golf","boxing",
            "cricket","test match","ashes","ipl",
            # Generic sports terms
            "win the","stanley cup","championship","playoffs","finals","title",
            "golden boot","mvp","relegated","promoted","top scorer",
            "ballon d'or","transfer","signing",
        ],
