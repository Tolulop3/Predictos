...
def data_api_price_history(market_id: str,
    ...
    """
    Fetch price history — tries CLOB prices-history first (most reliable),
    falls back to data-api /prices if available.
    Returns list of {t, p} or {t, c, v} dicts sorted oldest-first.
    """
    import time as _time
    end_ts   = int(_time.time())
    start_ts = end_ts - 4 * 86400  # 4 days back

    # Primary: CLOB prices-history (always works, no auth needed)
    data = _get(
    ...
    # Fallback: data-api (endpoint may vary by market)
    for endpoint in ["/prices", f"/prices/{market_id}"]:
        data = _get(
            f"{config.POLYMARKET_DATA_BASE}{endpoint}",
            params={"interval": interval, "fidelity": fidelity},
        )
        if data:
...
def fetch_manifold_markets(limit: int = 100) -> list[dict]:
    ...
    before  = None
    for _ in range(3):
        # Manifold v0 API: outcomeType as query param causes 400
        # Use only supported params: limit, filter, sort, before
        params = {"limit": limit, "filter": "open", "sort": "score"}
        if before:
