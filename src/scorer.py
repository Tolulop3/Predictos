...
def apply_filters(market: dict, sentiment: dict, model_prob: float,
    ...
    #    UNTRAINED fallback: require model to differ from market by ≥ 15% AND confidence ≥ 0.50
    #    This prevents the fallback from entering near-zero-edge trades while waiting for training
    yes_price = market.get("yes_price", 0.5)
    ...
    if model_is_trained:
        ...
        # Fallback: require meaningful divergence from market price
        edge_abs = abs(model_prob - yes_price)
        if edge_abs < 0.10:   # must see at least 10% probability divergence
            flags.append("insufficient_edge")
            passes = False
    ...
    # 5. ML confidence — hard block below minimum, soft flag above minimum but below target
    if ml_confidence < config.ML_MIN_CONFIDENCE_ENTRY:
        flags.append("low_ml_confidence")
        passes = False   # hard block — not enough signal agreement to enter
