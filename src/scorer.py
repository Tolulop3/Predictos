...
def apply_filters(market: dict, sentiment: dict, model_prob: float,
    ...
    # 4. Graham edge filter
    #    When ML model is trained: require GRAHAM_EDGE_THRESHOLD (6%)
    #    When untrained (fallback): require only 2% — live validator is the quality gate
    edge_abs      = abs(model_prob - market.get("yes_price", 0.5))
    edge_threshold = config.GRAHAM_EDGE_THRESHOLD if model_is_trained else 0.02
