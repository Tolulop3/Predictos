...
def score_decay(market: dict, momentum_score: float) -> float:
    ...
    Far-out markets with no momentum are penalised.
    """
    days = market.get("days_to_res", 30)

    # Optimal window: 3–14 days out with strong momentum
    if days < 0.1:
        return 0.0  # already closed
    if days < 2:
        return 30.0  # too close (binary event risk)

    # Time proximity score (closer = higher, up to 14 days)
    if days <= 14:
        proximity = (14 - days) / 14 * 60 + 20  # 20–80
    elif days <= 60:
        proximity = max(0, 20 - (days - 14) / 46 * 20)  # 0–20
    else:
        proximity = 0.0
...
def apply_filters(market: dict, sentiment: dict, model_prob: float,
    ...
    # 1b. Too far out: >120 days = too speculative, no meaningful signal yet
    if market.get("days_to_res", 0) > config.MAX_RESOLUTION_DAYS:
