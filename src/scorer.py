...
def apply_filters(market: dict, sentiment: dict, model_prob: float,
                  ...
                  n_total_picks: int = 999) -> tuple[bool, list[str]]:
    """
    ...
    # Pre-compute direction for entry rule
    yes_price_for_dir   = market.get("yes_price", 0.5)
    direction_for_filter = "YES" if model_prob > yes_price_for_dir else "NO"
    ...
    # 0b. Absolute probability floor — yes_price below 5% is never actionable
    # regardless of model_prob. Catches longshots that sneak past MIN_MARKET_PROB.
    yes_price_raw = market.get("yes_price", 0.5)
    if yes_price_raw < 0.05:
    ...
    # 1b. Too far out — uses config thresholds (180d general, 45d sports)
    ...
    # 4. Entry rule
    #    TRAINED model: enforce spec rule — market_price ≤ model_prob × 0.5 (2× minimum edge)
    #    UNTRAINED fallback: require ≥5% probability divergence
    #    Fallback model produces 2-8% edge max. 5% requires sentiment score ≥0.75 (strong signal).
    #    This blocks noise trades while allowing genuine news-backed picks through.
    yes_price = market.get("yes_price", 0.5)
    direction_probe = "YES" if model_prob > yes_price else "NO"

    if model_is_trained:
        # Spec rule: strict 2× edge requirement
        if direction_probe == "YES":
            entry_ok = yes_price <= model_prob * config.ENTRY_DISCOUNT
        else:
            no_price      = 1 - yes_price
            model_no_prob = 1 - model_prob
            entry_ok      = no_price <= model_no_prob * config.ENTRY_DISCOUNT
        if not entry_ok:
            flags.append("insufficient_edge")
            passes = False
    else:
        # Fallback edge check — threshold adapts to bootstrap phase.
        # Bootstrap (< 20 total picks): 1% edge — allows sentiment-only picks
        #   to flow through so the training store can accumulate data.
        # Normal (≥ 20 picks): 2% edge — requires momentum or strong sentiment.
        # Also requires at least 1 relevant article to avoid pure noise trades.
        edge_abs   = abs(model_prob - yes_price)
        has_news   = sentiment.get("article_count", 0) >= 1
        min_edge   = 0.01 if n_total_picks < 20 else 0.02
        if edge_abs < min_edge or not has_news:
            flags.append("insufficient_edge")
            passes = False
    ...
    # 5. ML confidence — hard block only at very low confidence (no signal agreement at all)
    #    Soft flag for moderate confidence (paper only, reduce size)
    #    Fallback model produces 0.40-0.57 range. Trained model can reach 0.80+.
    if ml_confidence < config.ML_MIN_CONFIDENCE_ENTRY:
        flags.append("low_ml_confidence")
        passes = False   # hard block — signals actively conflict
    elif ml_confidence < config.ML_CONFIDENCE_THRESHOLD:
        flags.append("low_ml_confidence")
        # soft flag — paper only, reduce size
...
def score_market(market: dict,
    ...
    # Filters
    passes, flags = apply_filters(
        market, sentiment_result, model_prob, ml_confidence,
...
def rank_markets(scored_markets: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    """
    import logging
    log = logging.getLogger("predictos.scorer")

    # Diagnostic: count why markets fail
    fail_counts: dict[str, int] = {}
    passed = 0
    for m in scored_markets:
        if m.get("passes_filter"):
            passed += 1
        for flag in m.get("flags", []):
            fail_counts[flag] = fail_counts.get(flag, 0) + 1

    top_fails = sorted(fail_counts.items(), key=lambda x: -x[1])[:6]
    log.info(f"Filter results: {passed}/{len(scored_markets)} passed | "
             f"Top blocks: {', '.join(f'{k}:{v}' for k,v in top_fails)}")
    yes_picks_raw = sorted(
    ...
    # Event deduplication: max 1 pick per event_id, max 2 per category
    # Also deduplicates by tournament name when markets aren't grouped under same event_id
    # (e.g. Masters golf — each player is a separate Polymarket event)
    def _deduplicate(picks: list[dict], max_per_event: int = 1,
                     max_per_category: int = 3) -> list[dict]:
        import re
        seen_events: dict[str, int] = {}
        seen_categories: dict[str, int] = {}
        # Extract tournament/competition keywords to catch ungrouped correlated markets
        seen_tournaments: set[str] = set()
        result = []

        # Keywords that identify a tournament context — 2+ picks from same = deduplicate
        TOURNAMENT_PATTERNS = [
            r'\b(masters|world cup|champions league|super bowl|stanley cup|nba finals'
            r'|world series|wimbledon|us open|french open|australian open'
            r'|ballon d.or|golden boot|mvp award|oscar|emmy|grammy'
            r'|colombian presiden|presidential election \d{4})\b'
        ]
        compiled = [re.compile(p, re.IGNORECASE) for p in TOURNAMENT_PATTERNS]

        def _tournament_key(question: str) -> str:
            for pat in compiled:
        ...
        for p in picks:
            event_id = p.get("event_id") or p.get("event_title") or ""
            category = p.get("category", "other")
            question = p.get("question", "")
            tournament = _tournament_key(question)

            # Block if same event already represented
            if event_id and seen_events.get(event_id, 0) >= max_per_event:
                continue
            # Block if same tournament keyword already seen
            if tournament and tournament in seen_tournaments:
                continue
            # Block if category has hit max
            ...
            if tournament:
                seen_tournaments.add(tournament)
            seen_categories[category] = seen_categories.get(category, 0) + 1
        return result

    yes_picks = _deduplicate(yes_picks_raw)
    no_picks  = _deduplicate(no_picks_raw)
