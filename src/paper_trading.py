...
def open_paper_trade(pick: dict, model_prob: float) -> Optional[dict]:
    ...
    # Block extreme markets — near-certainties AND longshots
    # yes_price > 0.92: market is 92%+ certain (near-certainty, already caught by scorer)
    # yes_price < 0.08: market is <8% (longshot — should be caught by scorer)
    # entry_price < 0.05: the specific side being bet is extremely unlikely
    # All three cases produce bad Kelly sizing and unreliable signal
    if yes_price > config.MAX_MARKET_PROB or yes_price < config.MIN_MARKET_PROB:
        return None
    if entry_price < 0.05:
        return None
