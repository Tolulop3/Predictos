...
def open_paper_trade(pick: dict, model_prob: float) -> Optional[dict]:
    ...
    # Block extreme markets — near-certainties AND extreme longshots
    # Use 5% absolute floor (matches scorer's absolute floor)
    # MAX_MARKET_PROB (92%) blocks near-certainties
    if yes_price > config.MAX_MARKET_PROB:
        return None
    if entry_price < 0.05:
        return None
