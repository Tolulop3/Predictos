...
def _backdate_single_ticks():
    """
    One-shot bootstrap migration.
    When the tick store has exactly 1 tick per market and that tick was
    written in the last 8 hours (same-day cache bootstrap), rewrite its
    timestamp to now-28h. This makes compute_tick_momentum see a real
    24h delta on the very next save_ticks_batch call, instead of waiting
    a full calendar day.
    Self-limiting: only fires when files have exactly 1 line < 8h old.
    After the first proper run (2 ticks per market), never triggers again.
    """
    if not TICK_DIR.exists():
        return
    now_ts  = int(time.time())
    cutoff  = now_ts - 8 * 3600   # "written today" threshold
    target  = now_ts - 28 * 3600  # backdate target: ~28h ago
    patched = 0
    for tick_path in TICK_DIR.rglob("*.jsonl"):
        try:
            with open(tick_path) as f:
                lines = [l for l in f if l.strip()]
            if len(lines) != 1:
                continue  # already has history — skip
            tick = json.loads(lines[0])
            if tick.get("ts", 0) < cutoff:
                continue  # already old enough — skip
            tick["ts"] = target
            ...
            pass
    if patched:
        log.info(f"Tick bootstrap: backdated {patched} single-tick files to enable 24h momentum")
