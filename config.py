...
# ─── Scoring Weights ──────────────────────────────────────────────────────────
PILLAR_WEIGHTS = {
    "momentum":  0.20,   # reduced — tick store needs more history to be reliable
    "volume":    0.20,
    "sentiment": 0.25,
    "edge":      0.20,
    "decay":     0.15,   # increased — penalises far-out markets harder
}

# ─── Signal Filters ───────────────────────────────────────────────────────────
MIN_RESOLUTION_HOURS      = 48    # skip markets resolving in <48h (binary event risk)
MAX_RESOLUTION_DAYS       = 60    # skip markets resolving >60 days out (too speculative)
MAX_RESOLUTION_DAYS_SPORTS = 30   # sports markets: max 30 days (season-long = too noisy)
MIN_DAILY_VOLUME_USD      = 5_000  # liquidity floor — lowered to get more NO picks
GRAHAM_EDGE_THRESHOLD     = 0.08  # 8% min diff: market prob vs model prob
MIN_MARKET_PROB           = 0.08  # ignore markets priced below 8%
MAX_MARKET_PROB           = 0.92  # ignore markets priced above 92%
NEWS_HALF_LIFE_DAYS       = 5
TREND_HALF_LIFE_DAYS      = 10
ML_CONFIDENCE_THRESHOLD   = 0.50  # lowered from 0.55 — gets more valid picks through
