...
class TestScorer(unittest.TestCase):
    ...
    def test_filter_graham_edge(self):
        ...
        # Trained model: 2x rule — model=0.55 on market=0.50 fails
        passes, flags = apply_filters(m, {"article_count": 5}, 0.55, 0.7, model_is_trained=True)
        self.assertFalse(passes)
        self.assertIn("insufficient_edge", flags)

        # Untrained fallback: 2% edge + news required
        # model=0.53, market=0.50 → 3% edge, 5 articles → PASS
        passes3, flags3 = apply_filters(m, {"article_count": 5}, 0.53, 0.7, model_is_trained=False)
        self.assertTrue(passes3)

        # model=0.51, market=0.50 → 1% edge → FAIL (below 2%)
        passes4, flags4 = apply_filters(m, {"article_count": 5}, 0.51, 0.7, model_is_trained=False)
        self.assertFalse(passes4)
        self.assertIn("insufficient_edge", flags4)

        # 3% edge but NO news → FAIL
        passes5, flags5 = apply_filters(m, {"article_count": 0}, 0.53, 0.7, model_is_trained=False)
        self.assertFalse(passes5)
        self.assertIn("insufficient_edge", flags5)
